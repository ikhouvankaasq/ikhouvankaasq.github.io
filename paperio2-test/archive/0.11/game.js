...
// We gebruiken het grid alleen nog voor de logica (onzichtbaar), 
// maar we tekenen het land als een vloeiend pad.
const TILE_SIZE = 10; 
...
class Player {
    ...
    update() {
        ...
        if (tile === 0) {
            ...
            if (this.isOutside && this.trail.length > 10) {
                const hit = this.trail.some((p, i) => i < this.trail.length - 10 && p.gx === gx && p.gy === gy);
    ...
    draw() {
        // --- DE "VLOEIENDE" OPLOSSING ---
        // We gebruiken een tijdelijk canvas of filter om de pixels te vervagen en dan weer scherp te maken
        // Dit heet de "Metaball" techniek en zorgt voor perfecte ronde vormen.
        ctx.save();
        ctx.filter = 'blur(4px) contrast(150%)'; // DIT HAALT DE PIXELS WEG

...
class Player {
        ...
        for (let x = -8; x <= 8; x++) {
            for (let y = -8; y <= 8; y++) {
                if (Math.sqrt(x*x + y*y) <= 8) {
                    if(startGX+x >= 0 && startGX+x < gridWidth && startGY+y >= 0 && startGY+y < gridHeight) {
                        territory[startGX+x][startGY+y] = 1;
                    }
                }
            }
        }
    ...
    draw() {
        // --- SMOOTH VECTOR LAND ---
        // We tekenen nu het land door de gridpunten als 1 groot vloeiend pad te vullen
        ctx.fillStyle = this.color;
        ctx.beginPath();
        for (let x = 0; x < gridWidth; x++) {
            for (let y = 0; y < gridHeight; y++) {
                if (territory[x][y] === 1) {
                    // We gebruiken kleine cirkels die elkaar overlappen om de randen 
                    // van de vorm perfect rond en glad te maken, net als de lijn.
                    ctx.moveTo(x * TILE_SIZE + TILE_SIZE, y * TILE_SIZE + TILE_SIZE/2);
                    ctx.arc(x * TILE_SIZE + TILE_SIZE/2, y * TILE_SIZE + TILE_SIZE/2, TILE_SIZE * 0.75, 0, Math.PI * 2);
                }
            }
        }
        ctx.fill();

        // --- TRAIL (SMOOTH VECTOR LINE) ---
